...
`awc_dashboard_streamlit.py` picks its backend at startup: if a `DATABASE_URL` is available, it queries Postgres (e.g. a Neon database) via `psycopg2`; otherwise it falls back to the local `awc_warehouse.sqlite`. `DATABASE_URL` is resolved from the OS environment variable first, then from `st.secrets["DATABASE_URL"]` (`_resolve_database_url` in `awc_dashboard_streamlit.py`) - Streamlit Community Cloud delivers secrets via `st.secrets`, not OS environment variables, so the env-var-only check used before this would have silently fallen back to SQLite on a Cloud deployment with the secret configured. The env var wins when both are present (useful for local dev overriding a `secrets.toml`); a missing `secrets.toml` (the normal local-dev state) is treated the same as no secret configured, not an error. Every page - including Anomaly Surveillance - works identically across all three states (env var, secrets, neither); SQL is written portably (no SQLite-only `strftime`/`substr`, dialect differences like `LIKE` vs `ILIKE` are chosen automatically) rather than branching per page.
